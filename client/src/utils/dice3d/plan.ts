...
export function planDice(rolls: DieRoll[]): DieInstance[] {
  ...
  for (const { sides, value } of rolls) {
    if (!isAnimatable(sides)) continue
    if (sides === 100) {
      dice.push({ sides: 10, kind: 'tens', faceIndex: Math.floor((value - 1) / 10) })
      dice.push({ sides: 10, kind: 'normal', faceIndex: (value - 1) % 10 })
    } else {
      dice.push({ sides, kind: 'normal', faceIndex: value - 1 })
    }
  }
