 ...
 * encre claire sur un dé sombre. Luminance relative (WCAG), seuil à 0,3 : le
 * safran passe en sombre, tous les autres tons de la palette en clair.
 */
export function inkFor(hex: string): string {
  const value = hex.replace('#', '')
  if (value.length !== 6) return INK_DARK
  const channel = (i: number) => {
    const c = parseInt(value.slice(i, i + 2), 16) / 255
    return c <= 0.03928 ? c / 12.92 : ((c + 0.055) / 1.055) ** 2.4
  }
  const luminance = 0.2126 * channel(0) + 0.7152 * channel(2) + 0.0722 * channel(4)
  return luminance > 0.3 ? INK_DARK : INK_LIGHT
}
