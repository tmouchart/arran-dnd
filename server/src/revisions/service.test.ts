 ...
 *
 * Sans URL de base (CI), tout le fichier est ignoré : il n'y a pas de Postgres
 * à joindre et le déploiement ne doit pas échouer pour ça.
 */
const hasDb = Boolean(getDatabaseUrl())

describe.runIf(hasDb)('révisions (intégration base)', () => {
  let userId: number
  let noteId: number
